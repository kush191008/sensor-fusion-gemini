...
Validates multi-scenario telemetry generation, Gemini diagnostics, Copilot,
Audit Report generation, Cyber-Attack injection, and Self-Healing Firmware synthesis.
"""
...
from sensor_simulator import generate_sensor_stream, MissionScenario, CyberAttackType
...
class TestSensorFusionPipeline(unittest.TestCase):
    ...
    def test_cyber_attack_injection(self):
        """Verify Chaos Monkey cyber-physical attack simulation."""
        for attack in [CyberAttackType.EMI_SURGE, CyberAttackType.SPOOFING, CyberAttackType.CRYO_FREEZE]:
            df_attack = generate_sensor_stream(n_samples=400, attack=attack)
            self.assertEqual(len(df_attack), 400)
            self.assertFalse(df_attack.isna().any().any())
    ...
    def test_gemini_analyzer_diagnostics(self):
        ...
        # Humidity transducer should be detected as FAILED
        self.assertEqual(self.diagnostics['sensor_humidity']['status'], 'FAILED')
        # Temperature transducer should have drift detected
    ...
    def test_firmware_patch_synthesis(self):
        """Verify autonomous embedded C and MicroPython firmware patch generation."""
        context = {
            'scenario_title': 'Industrial Gas Turbine',
            'diagnostics': self.diagnostics
        }
        patch = self.analyzer.generate_firmware_patch(context)
        self.assertIn("c_code", patch)
        self.assertIn("micropython_code", patch)
        self.assertIn("#include", patch["c_code"])
        self.assertIn("def read_channel_temp", patch["micropython_code"])

    def test_fft_spectral_analysis(self):
        """Verify FFT spectral decomposition and SNR analysis."""
        sig = self.df['sensor_aux'].values
        fft_data = self.analyzer.analyze_frequency_spectrum(sig)
        self.assertIn("peak_freq", fft_data)
